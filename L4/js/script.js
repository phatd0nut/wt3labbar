...
function ImageViewer(id) {
	...
	ImageViewer.prototype.requestImages = function (file) {
		...
		request.onreadystatechange = function () {
			if (request.readyState == 4)
				if (request.status == 200) self.getImages(request.responseXML);
	...
	ImageViewer.prototype.getImages = function (XMLcode) {
		...
		let urlElems = XMLcode.getElementsByTagName("url");
		let captionElems = XMLcode.getElementsByTagName("caption");
		this.list = [];
		for (let i = 0; i < urlElems.length; i++) {
			this.list.push({ url: urlElems[i].firstChild.data, caption: captionElems[i].firstChild.data });
		}
		imgIx = 0;
		this.showImage();
	}
